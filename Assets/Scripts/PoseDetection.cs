...
public class PoseDetection : MonoBehaviour
{
    ...
    bool m_HasActiveDetectAwaitable;
    bool m_IsDestroying;
    ...
    {
        if (imageTexture == null || poseDetector == null || poseLandmarker == null || anchorsCSV == null || posePreview == null || imagePreview == null)
        {
            Debug.LogError("PoseDetection is missing one or more required references.");
            return;
        }
        ...
        while (true)
        {
            ...
            try
            {
                ...
                m_HasActiveDetectAwaitable = true;
                await m_DetectAwaitable;
                m_HasActiveDetectAwaitable = false;
            }
    ...
    void OnDestroy()
    {
        ...
        {
            // Unity can destroy this component before the Awaitable is fully initialized.
        }
    }
